...
DEFAULT_MONITOR_SCHEDULE = "every 30m"
MONITOR_JOB_NAME_PREFIX = "goals-monitor:"
...
def _monitor_job_name(alias: GoalAlias) -> str:
    return f"{MONITOR_JOB_NAME_PREFIX}{alias.alias}"


def _safe_script_slug(value: str) -> str:
    slug = re.sub(r"[^A-Za-z0-9_.-]+", "-", value).strip(".-")
    return slug or "goal"


def _goals_monitor_scripts_dir() -> Path:
    from hermes_constants import get_hermes_home

    return Path(get_hermes_home()) / "scripts"


def _write_monitor_script(alias: GoalAlias, repo_path: Path) -> str:
    scripts_dir = _goals_monitor_scripts_dir()
    scripts_dir.mkdir(parents=True, exist_ok=True)
    script_name = f"goals-monitor-{_safe_script_slug(alias.alias)}.py"
    script_path = scripts_dir / script_name
    source_root = Path(__file__).resolve().parents[1]
    repo_roots = str(repo_path.resolve().parent)
    monitor_command = f"/goals monitor {alias.alias}"
    script_path.write_text(
        "\n".join(
            [
                "#!/usr/bin/env python3",
                '"""Generated by Hermes /goals schedule. Do not add secrets here."""',
                "import os",
                "import sys",
                "",
                f"sys.path.insert(0, {str(source_root)!r})",
                f"os.environ.setdefault('HERMES_GOALS_REPO_ROOTS', {repo_roots!r})",
                "",
                "from hermes_cli.repo_goals import handle_goals_command",
                "",
                "if __name__ == '__main__':",
                f"    print(handle_goals_command({monitor_command!r}))",
            ]
        )
        + "\n",
        encoding="utf-8",
...
def schedule_goals_monitor(
    alias: GoalAlias,
    schedule: str = DEFAULT_MONITOR_SCHEDULE,
    *,
    origin: Optional[Mapping[str, object]] = None,
    deliver: Optional[str] = None,
) -> tuple[dict, bool, str]:
    repo_path = resolve_repo_path(alias)
    script_name = _write_monitor_script(alias, repo_path)
    job_name = _monitor_job_name(alias)
    prompt = f"Monitor repo-local goal `{alias.alias}` and post the repo-generated report packet."
    deliver_value = deliver or ("origin" if origin else "local")

    from cron.jobs import create_job, list_jobs, update_job

    existing = next(
        (job for job in list_jobs(include_disabled=True) if job.get("name") == job_name),
        None,
    )
    if existing:
        job = update_job(
        ...
        if job is None:
            raise RepoGoalsError(f"Could not update scheduled monitor for `{alias.alias}`.")
        return job, True, script_name
    ...
    return job, False, script_name


def unschedule_goals_monitor(alias: GoalAlias) -> Optional[dict]:
    from cron.jobs import pause_job

    return pause_job(_monitor_job_name(alias), reason="Paused by /goals unschedule")


def _render_monitor_schedule(alias: GoalAlias, job: dict, updated: bool, script_name: str) -> str:
    action = "Updated scheduled monitor" if updated else "Scheduled monitor"
    lines = [
        f"{action} for `/goals monitor {alias.alias}`.",
        f"Cron job: `{job.get('name')}` (`{job.get('id')}`)",
        f"Schedule: {job.get('schedule_display') or '?'}",
        f"Delivery: {job.get('deliver') or 'local'}",
        f"Script: `{script_name}`",
    ]
    if job.get("next_run_at"):
        lines.append(f"Next run: {job['next_run_at']}")
    lines.append(f"Manual check: /goals monitor {alias.alias}")
    return "\n".join(lines)
...
def render_slack_summary(alias: str, report: str, status_text: str = "", start_text: str = "") -> str:
    sections = {
        ...
        "Deliverables": extract_report_section(report, "Deliverables"),
        "Expected Artifacts": extract_report_section(report, "Expected Artifacts"),
        "Log Errors": extract_report_section(report, "Log Errors"),
    ...
    if _expected_artifacts_are_missing(sections["Expected Artifacts"]):
        lines.extend(["", "Monitor State", "Incomplete: expected artifacts are missing."])
    if status_text:
...
def _expected_artifacts_are_missing(section: str) -> bool:
    if not section:
        return False
    ...
    for line in lines:
        ...
        if lower.startswith("missing:"):
            ...
            if trailing and trailing.lower() not in {"none", "n/a", "0"}:
                return True
            continue
        if in_missing:
            if lower.startswith(("present:", "log", "errors:", "deliverables:")):
                return False
            if stripped.startswith("-") and stripped not in {"-", "- none"}:
                return True
            if stripped and lower not in {"none", "n/a"}:
                return True
    return False
...
def handle_goals_command(
    command: str,
    aliases: Optional[Mapping[str, GoalAlias]] = None,
    *,
    origin: Optional[Mapping[str, object]] = None,
    deliver: Optional[str] = None,
) -> str:
    ...
    if action in {"list", "ls", "help"}:
        return _format_alias_list(aliases)
    if len(tokens) < 3:
        return "Usage: /goals [start|status|report|monitor|schedule|unschedule|pause|resume|stop] <alias> [--input path/to/export.csv]"
    ...
    try:
        ...
        if action in {"schedule", "watch"}:
            schedule = " ".join(tokens[3:]).strip() or DEFAULT_MONITOR_SCHEDULE
            job, updated, script_name = schedule_goals_monitor(alias, schedule, origin=origin, deliver=deliver)
            return _render_monitor_schedule(alias, job, updated, script_name)

        if action in {"unschedule", "unwatch"}:
            job = unschedule_goals_monitor(alias)
            if not job:
                return f"/goals `{action} {alias.alias}` blocked\nNo scheduled monitor found for `{alias.alias}`."
            return (
                f"Paused scheduled monitor for `/goals monitor {alias.alias}`.\n"
                f"Cron job: `{job.get('name')}` (`{job.get('id')}`)"
            )
